...
Zero-knowledge proof systems such as Groth16 (Groth, 2016), PLONK (Gabizon et al., 2019), and related zk-SNARK constructions provide succinct and efficiently verifiable proofs of correct computation over arithmetic circuits. These systems allow a prover to convince a verifier that a computation was executed correctly without revealing private inputs.

More recently, practical toolchains such as libsnark (Ben-Sasson et al., 2014), Halo2 (Bowe et al., 2020), and EzKL (2023) have enabled developers to compile high-level programs and machine learning models into circuits suitable for zk-SNARK proving.
...
Zero-knowledge machine learning focuses on proving correct execution of machine learning inference without revealing private inputs. zkCNN (Liu et al., 2021) demonstrates how convolutional neural network predictions and accuracy can be verified in zero knowledge by expressing inference as arithmetic circuits. Similarly, vCNN: Verifiable Convolutional Neural Network Based on zk-SNARKs presents a framework for generating zk-SNARK proofs that certify correct execution of convolutional neural network inference.
...
Research in privacy-preserving machine learning includes techniques such as secure multiparty computation (Lindell \& Pinkas, 2009), homomorphic encryption for neural network inference (Gilad-Bachrach et al., 2016; Brutzkus et al., 2019), and confidential computing enclaves.
...
AI governance frameworks emphasize auditing, transparency, and accountability. Model cards (Mitchell et al., 2019), datasheets for datasets (Gebru et al., 2018), and regulatory frameworks such as the EU AI Act propose documentation-based or process-based accountability.
...
Personal data detection is commonly performed using rule-based systems (regex patterns), named entity recognition models, or hybrid approaches (Dernoncourt et al., 2017). Production systems for content moderation rely on deterministic filters, ML classifiers, and policy enforcement pipelines.
...
Because the circuit operates on a fixed-size input encoding ($L = 2048$ bytes), LLM outputs longer than $L$ are truncated by the pipeline before attestation; outputs shorter than $L$ are zero-padded. The bound $L$ is chosen so that the \emph{whole} (capped) output is encoded and attested in a single proof, avoiding chunking. As a result, the constraint system remains constant across experiments, explaining the stability of proof size and runtime.
...
Second, the circuit operates on fixed-length inputs. Outputs are capped at $L$ so that a single proof attests to the entire string; longer responses would require chunking, multiple proofs, or recursive composition to remain practical.
...
\begin{enumerate}
    \item Groth, J. (2016). \emph{On the Size of Pairing-Based Non-interactive Arguments}. EUROCRYPT.
    \item Gabizon, A., Williamson, Z., \& Ciobotaru, O. (2019). \emph{PLONK: Permutations over Lagrange-bases for Oecumenical Noninteractive Arguments of Knowledge}.
    \item Ben-Sasson, E., et al. (2014). \emph{Succinct Non-Interactive Zero Knowledge for a von Neumann Architecture}.
    \item Liu, J., et al. (2021). \emph{zkCNN: Zero Knowledge Proofs for Convolutional Neural Network Predictions}.
    \item Lee, S., Ko, H., Kim, J., \& Oh, H. (2024). \emph{vCNN: Verifiable Convolutional Neural Network Based on zk-SNARKs}. IEEE Transactions on Dependable and Secure Computing, 21(4), 4254--4270.
    \item Lindell, Y., \& Pinkas, B. (2009). \emph{Secure Multiparty Computation for Privacy-Preserving Data Mining}.
    \item Gilad-Bachrach, R., et al. (2016). \emph{CryptoNets: Applying Neural Networks to Encrypted Data}.
    \item Brutzkus, A., et al. (2019). \emph{Low Latency Privacy Preserving Inference}.
    \item Mitchell, M., et al. (2019). \emph{Model Cards for Model Reporting}.
    \item Gebru, T., et al. (2018). \emph{Datasheets for Datasets}.
    \item Dernoncourt, F., et al. (2017). \emph{De-identification of patient notes with recurrent neural networks}.
\end{enumerate}
